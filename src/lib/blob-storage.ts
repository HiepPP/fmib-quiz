...
export const blobStorage = {
  ...
  saveQuestions: async (
    ...
    try {
      ...
      console.log(
        `✅ Successfully saved ${questions.length} questions via API`,
      );
      console.log(
        `📁 Blob file created/updated at: ${result.data?.url || "api-success"}`,
      );
      return {
        url: result.data?.url || "api-success",
        uploadedAt: result.data?.uploadedAt
          ? new Date(result.data.uploadedAt)
          : new Date(),
      };
    } catch (error) {
      const errorMessage =
      ...
      if (errorMessage.includes("401") || errorMessage.includes("403")) {
        console.error(
          "🔑 Permission denied - Check your BLOB_READ_WRITE_TOKEN",
        );
      } else if (errorMessage.includes("404")) {
        console.error(
          "🏪 Blob store not found - Create a blob store in Vercel dashboard",
        );
      } else if (
      ...
      throw new BlobStorageError(
        "Failed to save questions via API",
        error as Error,
      );
    }
  },
  ...
  getQuestions: async (): Promise<Question[]> => {
    ...
    const result = await response.json();
    console.log("API GET /blob-questions result:", result);
    ...
    try {
      ...
      console.log(
        `✅ Successfully loaded ${questions.length} questions via API (${result.source})`,
      );
      return questions;
    } catch (error) {
      const errorMessage =
  ...
  deleteQuestions: async (): Promise<void> => {
    try {
      ...
      if (!response.ok) {
        const result = await response.json();
        throw new Error(
          result.error || result.message || "Failed to delete questions",
        );
      }
      ...
      throw new BlobStorageError(
        "Failed to delete questions via API",
        error as Error,
      );
    }
  },
